...
{
    public partial class AdminAcount : Form
    {
        public AdminAcount()
        {
            InitializeComponent();
        ...
        private void AdminAcount_Load(object sender, EventArgs e)
        {
                ...
                using (SqlConnection connExpDtls = new SqlConnection(ConfigurationManager.ConnectionStrings["Game_Rental.Properties.Settings.Game_RentalConnectionString"].ConnectionString))
                {
                    using (SqlCommand cmdExpDtls = new SqlCommand())
                    {
                        try
                        {
                            ...
                            cmdExpDtls.CommandText = cmdExpDtls.CommandText + " update Admin set Fname = @Fname,Lname = @Lname,email = @email,phone = @phone,password = @password ";
                            ...
                            cmdExpDtls.Parameters.Add("@phone", SqlDbType.Int).Value = textBox4.Text;
                            ...
                            string Messages = "";
                            Messages = Messages.ToString() + "Updated Successfully";
